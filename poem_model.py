...
class PoetryModel(object):
    def __init__(self, config):
        self.model = None # 保存模型
        self.do_train = True # 是否已经进行了训练
        self.loaded_model = True # 是否已经加载了模型
        self.config = config
        ...
        if os.path.exists(self.config.weight_file) and self.loaded_model:
            ...
            self.do_train = False
            print("Can't find model file,you should train the model first!")
    ...
    def _pred(self, sentence, temperature=1):
        ...
        if len(sentence) < self.config.max_len:
            ...
            return
        # 对 sentence 进行截断，只取倒数max_len个字符
        sentence = sentence[-self.config.max_len:]
